...
/**
 * @ClassName OAuth2PasswordAuthenticationProvider
 * @Description TODO
 * @Author sky
 * @Date 2023/5/18 9:43
 * @Version 1.0
 **/
import com.sky.util.OAuth2AuthenticationProviderUtils;
...
public class OAuth2PasswordAuthenticationProvider implements AuthenticationProvider {
    ...
    private OAuth2AuthenticationException oAuth2AuthenticationException(Exception authenticationException) {
        if (authenticationException instanceof UsernameNotFoundException) {
            return new OAuth2AuthenticationException("ACCOUNT_NOT_EXIST");
        }
        if (authenticationException instanceof BadCredentialsException) {
            return new OAuth2AuthenticationException("USERNAME_OR_PASSWORD_INCORRECT");
        }
        if (authenticationException instanceof LockedException) {
            return new OAuth2AuthenticationException("ACCOUNT_FROZEN");
        }
        if (authenticationException instanceof AccountExpiredException) {
            return new OAuth2AuthenticationException("ACCOUNT_EXPIRED");
        }
        if (authenticationException instanceof CredentialsExpiredException) {
            return new OAuth2AuthenticationException("PASSWORD_EXPIRED");
        }
        if (authenticationException instanceof InternalAuthenticationServiceException) {
            if (authenticationException.getCause() instanceof DisabledException) {
                return new OAuth2AuthenticationException("ACCOUNT_DISABLE");
            }
        }
        return new OAuth2AuthenticationException("IDENTITY_VERIFICATION_FAILED");
    }
}
